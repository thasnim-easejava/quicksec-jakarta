...
// The above servletConfig is not supported in Liberty so changing from tWAS.


@ApplicationScoped

//@DeclareRoles({"OtherLogin_WebUsers","RunAsSpecified_servlet"})
@RunAs("RunAsSpecified_servlet")
@WebServlet(name="QuickSecForm",urlPatterns={"/webclient","/QuickSecForm"})
//@ServletSecurity(value=@HttpConstraint(rolesAllowed={"OtherLogin_WebUsers"},transportGuarantee=ServletSecurity.TransportGuarantee.CONFIDENTIAL), httpMethodConstraints={@HttpMethodConstraint(value="POST",rolesAllowed={"delegateUsers"})})
@ServletSecurity(value=@HttpConstraint(rolesAllowed={"OtherLogin_WebUsers"},transportGuarantee=ServletSecurity.TransportGuarantee.CONFIDENTIAL))

//Added for DB ID Store 
@Resource(name="jdbc/dbis", type=javax.sql.DataSource.class, mappedName="jdbc/dbis")
...
@FormAuthenticationMechanismDefinition(loginToContinue=@LoginToContinue(loginPage="/svtlogin.html", errorPage="/FormLoginErrPage.jsp", useForwardToLogin=false))

/*
@LdapIdentityStoreDefinition(
        url = "ldap://isdsldapsvt1.rtp.raleigh.ibm.com:389/",
        callerBaseDn = "o=ibm,c=us",
        callerSearchBase = "ou=WebSphere,o=ibm,c=us",
        ...
        callerNameAttribute = "uid",
        	groupSearchFilter = "(|(objectclass=groupOfNames)(objectclass=groupOfUniqueNames)(objectclass=groupOfURLs))",
        groupNameAttribute = "dn",
        priority=1  //default is 80
	
     ) */

//groupNameAttribute = "cn",
//groupSearchFilter = "(&(cn=%v)(|(objectclass=groupOfNames)(objectclass=groupOfUniqueNames)(objectclass=groupOfURLs)))",

// Currently the dbpersona2/pwd will work for database login

/*@DatabaseIdentityStoreDefinition(	    
		dataSourceLookup="java:comp/env/jdbc/dbis",
		// dataSourceLookup="jdbc/dbis",
	    callerQuery="select password from dbusers where name = ?",
	    groupsQuery="select group_name from dbgroups where user_name = ?") 

*/

public class QuickSecForm extends HttpServlet  {
	...
	{			
		...
		//securityContext1 = null;
		
		timeStamp = (String) req.getSession().getAttribute("LOGINTIME");
		...
		try {
			...
			//JSR375 EE8 feature allows the use of security context for getting user name.
			...
			if(securityContext.getCallerPrincipal().getName()!=null)
				out.println("<font size=\"2\"><strong> Welcome user: " + securityContext.getCallerPrincipal().getName()+ "</strong></font>");
			...
			String securl =res.encodeURL("/QuickSecForm/secSession");
			String unsecurl =res.encodeURL("/QuickSecBasic/unsecSession");
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		try
		{
			...
			// Below denied methods are executed if print_out is true. Otherwise, it causes a lot of messages for longruns.			
